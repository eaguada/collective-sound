<template>
    <v-navigation-drawer v-model="drawer" fixed clipped app dark>
        <v-list dense>
            ...
            <v-subheader v-if="sc.auth.isGranted('Permissions_Administration')" class="mt-3 grey--text text--darken-1">{{$t('Administration').toUpperCase()}}</v-subheader>
            <v-list v-if="sc.auth.isGranted('Permissions_Administration')">
