<template>
    <div>
        <v-toolbar flat color="white">
            ...
            <v-spacer></v-spacer>
            <v-btn v-if="sc.auth.isGranted('Permissions_User_Create')" @click="editUser()" color="primary" dark class="mb-2">{{$t('NewUser')}}</v-btn>
        ...
        <v-data-table :headers="headers"
            ...
            <template v-slot:item.action="{ item }">
                <v-icon v-if="sc.auth.isGranted('Permissions_User_Update')"
                        small
                        class="mr-2"
                        @click="editUser(item.id)">
                    mdi-pencil
                </v-icon>
                <v-icon v-if="sc.auth.isGranted('Permissions_User_Delete')"
                        small
