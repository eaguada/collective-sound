...
let UserListComponent = class UserListComponent extends SCComponentBase {
    constructor() {
        super(...arguments);
        this.refs = this.$refs;
        this.loading = true;
    ...
    editUser(id) {
        ...
        this.errors = [];
        this.scService.get('/api/users/' + (id ? id : Guid.empty))
            .then((response) => {
    ...
    deleteUser(id) {
        this.swalConfirm(this.$t('AreYouSureToDelete').toString())
            .then((result) => {
            if (result.value) {
                this.scService.delete('/api/users?id=' + id)
                    .then((response) => {
                    debugger;
                    if (!response.isError) {
    ...
    save() {
        if (this.refs.form.validate()) {
            this.errors = [];
            this.scService.postOrPut('/api/users', this.createOrUpdateUserInput, this.createOrUpdateUserInput.user.id)
                .then((response) => {
                if (!response.isError) {
                    this.swalToast(2000, 'success', this.$t('Successful').toString());
                    this.dialog = false;
    ...
    getUsers() {
        ...
        this.scService.get('/api/users' + query, false).then((response) => {
