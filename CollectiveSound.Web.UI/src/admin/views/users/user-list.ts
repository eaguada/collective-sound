...
@Component
export default class UserListComponent extends SCComponentBase {
    ...
    editUser(id: string) {
        ...
        this.errors = [];
        this.scService.get<IGetUserForCreateOrUpdateOutput>('/api/users/' + (id ? id : Guid.empty))
            .then((response) => {
    ...
    save() {
        if (this.refs.form.validate()) {
            this.errors = [];
            this.scService.postOrPut<void>('/api/users',
    ...
    getUsers() {
        ...
        this.scService.get<IPagedList<IPagedListInput>>('/api/users' + query, false).then((response) => {
