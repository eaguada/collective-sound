<template>
    <div>
        <v-toolbar flat color="white">
            ...
            <v-spacer></v-spacer>
            <v-btn v-if="sc.auth.isGranted('Permissions_Role_Create')" @click="editRole()" color="primary" dark class="mb-2">{{$t('NewRole')}}</v-btn>
        ...
        <v-data-table :headers="headers"
            ...
            <template v-slot:item.action="{ item }">
                <v-icon v-if="!item.isSystemDefault && sc.auth.isGranted('Permissions_Role_Update')"
                        small
                        class="mr-2"
                        @click="editRole(item.id)">
                    mdi-pencil
                </v-icon>
                <v-icon v-if="!item.isSystemDefault && sc.auth.isGranted('Permissions_Role_Delete')"
                        small
