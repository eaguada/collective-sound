...
let RoleListComponent = class RoleListComponent extends SCComponentBase {
    constructor() {
        super(...arguments);
        this.refs = this.$refs;
        this.allPermissions = [];
    ...
    editRole(id) {
        ...
        this.errors = [];
        this.scService.get('/api/roles/' + (id ? id : Guid.empty))
            .then((response) => {
    ...
    deleteRole(id) {
        this.swalConfirm(this.$t('AreYouSureToDelete').toString())
            .then((result) => {
            if (result.value) {
                this.scService.delete('/api/roles?id=' + id)
                    .then((response) => {
                    if (!response.isError) {
    ...
    save() {
        if (this.refs.form.validate()) {
            this.errors = [];
            this.scService.postOrPut('/api/roles', this.createOrUpdateRoleInput, this.createOrUpdateRoleInput.role.id)
                .then((response) => {
                if (!response.isError) {
                    this.swalToast(2000, 'success', this.$t('Successful').toString());
                    this.dialog = false;
    ...
    getRoles() {
        ...
        this.scService.get('/api/roles' + query, false).then((response) => {
