...
@Component
export default class RoleListComponent extends SCComponentBase {
    ...
    editRole(id: string) {
        ...
        this.errors = [];
        this.scService.get<IGetRoleForCreateOrUpdateOutput>('/api/roles/' + (id ? id : Guid.empty))
            .then((response) => {
    ...
    save() {
        if (this.refs.form.validate()) {
            this.errors = [];
            this.scService.postOrPut<void>('/api/roles',
    ...
    getRoles() {
        ...
        this.scService.get<IPagedList<IRoleListOutput>>('/api/roles' + query, false).then((response) => {
